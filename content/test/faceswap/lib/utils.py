...
class GetModel():
    ...
    def get(self):
        ...
        if self._model_exists:
            ...
            return
        if os.path.exists(self._model_zip_path) is False:
            self.download_model()
            self.logger.debug("Zip File not exists, Download to: %s", self._model_zip_path)
        self.unzip_model()
    ...
    def unzip_model(self):
        ...
        try:
            ...
            try:
                zip_file = gzip.GzipFile(self._model_zip_path, "r")
                self.write_model(zip_file)
            except Exception as exceptErro:
                self.logger.error("Unable to extract model file: %s", str(exceptErro))
                # exit(1)
    ...
    def write_model(self, zip_file):
        ...
        pbar = tqdm(desc="Decompressing",
                    ...
                    total=length,
                    unit_scale=True)
        # remove param unit_divisor=1024
        for fname in fnames:
